...
- **One work item per branch.** Build one item at a time; never stack a second item's commits on the first item's branch.
...
- After a merge, the close-out (the human sets `status: shipped`) is the last step. A superseded or abandoned item goes `archived`.
