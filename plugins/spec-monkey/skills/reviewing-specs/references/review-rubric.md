...
- **Every boundary gets its own authored artifact:** *Artifacts to author* must carry a distinct line for each real boundary the change crosses (a database, an HTTP call, a queue, a module-to-module contract), not fold it inside a "unit tests" line. A boundary-crossing behavior with no dedicated artifact line is a BLOCKING gap — it is exactly the test that gets skipped. An artifact that needs infrastructure to run is still listed, tagged NEEDS INFRA, never omitted.
- **Every SC has an authored artifact:** each `SC-NNN` is proven by at least one line under *Artifacts to author*, or is named under *Coverage exceptions* with a reason. An SC with neither is a BLOCKING gap.
