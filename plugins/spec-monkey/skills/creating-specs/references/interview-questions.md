...
This is the discovery instrument for a spec. It is **not** the spec's layout: it follows the order you interrogate in, working backwards from every field the template needs filled. Discover here; compose the answers into `spec-template.md` afterward.
...
- **One-decision gate: settle this before going deep.** Is this exactly one independent decision?
  ...
  - If any of these is "yes", **stop and split**: a parent for the goal, shared contract, and orchestration, plus one child per decision. Interview each child on its own.
...
Work every lens. Each risk gets a decision: HANDLE (→ an FR), ACCEPT (→ Known limitations & honest gaps), or OUT-OF-SCOPE (where does it live?).
...
- **Better way:** is there a simpler or safer approach? Name it, then adopt it or record why this approach wins.
