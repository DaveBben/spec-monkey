...
- **The intent** (the plan file at `<plan_dir>/<slug>/plan.md`, default `.tests-as-specs/plans/<slug>/plan.md`): goal, the pointer to the contract tests, verification not covered by tests, out-of-scope. The intent steers; the **contract tests** bind. If you can't find the plan file, ask the human for it; don't audit against a guess.
