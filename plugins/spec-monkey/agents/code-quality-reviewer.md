...
- `spec_path` (optional): read its Approach, Constraints, and Edge Cases to see what the code must
...
5. **Brittle parsing.** Regex / string-splitting to extract from HTML/XML/JSON/YAML/ASTs. It breaks
...
8. **Error swallowing.** Catch-all that logs-and-returns-default, or an ignored returned error.
   Both mask bugs as "graceful degradation." Narrow to the specific failures the callee produces.
