...
- **The constitution** (`standards.md` / `CLAUDE.md` / `AGENTS.md`): the house rules the contract must respect.
...
**If the repo ships the spec linter** (`tools/spec-lint.py`), run it first and clear its errors before you read for judgment. It settles the mechanical checks a script does better than tokens — unfilled placeholders, ID uniqueness, FR↔SC pairing, status and profile values, the gate record — so your attention goes to the sweep and the verification. It's an optional convenience, not a dependency: on a harness or repo without it, do those checks by hand under the rubric.
...
5. **No restated fact**: each fact stated once and referenced by ID, not repeated across 3+ sections, and a constitution house rule cited rather than copied.
6. **No internal contradiction**: no value stated two ways, no assumption that fights a requirement.

If the contract has quietly grown a second independent decision the design didn't carry, that is a BLOCKING finding to split it — route it back through `reviewing-designs`.
...
2. Altitude & completeness gate: no HOW leaked, every `< >` answered, every SC testable, and no constitution house rule violated.
...
The default review reports and stops, and you **never rewrite**; that stands. When the user asks you to close the loop, and only then: hand the findings to the author (the human, or `writing-specs`, or `ideation` when a finding is really about the approach), let them revise, then review the revised contract and re-issue the verdict. You review; you do not author. Resolve BLOCKING findings, or have the human consciously accept one, before the verdict turns APPROVE. A reviewer who both writes and grades the spec is no longer an independent check.
...
- REVISE: hand back to `writing-specs` to resolve the blocking findings (or `ideation` when a finding is really about the approach, which then re-enters `reviewing-designs`), then re-review.
