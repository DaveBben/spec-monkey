...
- **FR-002**: the system SHALL record the source `dataset_id` and `source_revision` on the Dataset version
  it produces. *(records the provenance a downstream ModelArtifact needs — this FR makes it recordable)*
...
- **FR-005**: WHEN run again with the same `dataset_id`, `source_revision`, and `seed`, the system SHALL
  produce an identical example-to-split assignment.
...
**Constraints**: the output is the **Dataset** contract defined below in *Data & interface contract*. The
stack and storage limits are the repo's house rules (the constitution, `AGENTS.md`): Python 3.12+ with
PyTorch, and datasets live on the local on-disk registry, not cloud object storage. Local to this work item:
the max text length is a fixed config value, not a per-run flag.

**Non-functional bounds**: no raw example `text` appears in logs or the manifest (a house rule); a
100,000-row dataset splits in under 2 minutes on the single GPU node.

## Data & interface contract

- **Dataset**: a versioned, on-disk snapshot of labeled examples this loader writes, read by the trainer
  (train/val) and the evaluator (test). Per example `{id, text, label}`, where `label` is one of the known
  set (`pos` | `neg`). Invariants: every example belongs to exactly one split of a given version, and every
  `label` is in the known set.
- **SplitManifest** (new): the per-version index this loader writes — `dataset_id`, `source_revision`,
  ...
  the evaluator to locate and trust a version.
...
**Ordering & dependencies**: no upstream dependency (`depends_on: []`). Downstream, the trainer
...
a ModelArtifact trains on it, since that artifact records the exact version it trained on.
...
- The balance bound (FR-006) assumes single-label examples; multi-label datasets are out of scope for this
  loader, and FR-006 is undefined for them.
...
- [ ] Author a no-leak test: assert no raw example `text` appears in any emitted log line or in the
      SplitManifest. Proves the no-raw-text-in-logs house rule. Runs here.

**Gates to pass.**
- Runs here: `uv run pytest tests/test_loader.py` — covers SC-001..006 and the no-raw-text-in-logs bound.
