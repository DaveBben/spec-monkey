...
metadata:
  ...
  namespaced-as: "tests-as-specs:scoping"
---
...
**Read the past decisions.** Read the ADR files under `adr_dir` (`.tests-as-specs/config.json`, default `.tests-as-specs/adrs`) for any decisions touching this area. Carry them into the interview: don't re-open a settled question or contradict a constraint the team already chose. A past decision you'd overturn is one to raise, not to quietly reverse.
...
The tests carry the requirements now, so the intent stays short: steering and a table of contents, not a second copy of the spec. Write it to a **plan file** at `<plan_dir>/<slug>/plan.md`, where `plan_dir` comes from `.tests-as-specs/config.json` (default `.tests-as-specs/plans`) and `<slug>` is a short kebab-case name for the change (the branch name works). Format it with the template at `plan_template` (default `.tests-as-specs/plan-template.md`). The plan file is per-change scratch: gitignored by default, on disk for `building-from-plans` and `verifying-work` to read from a fresh context, deleted at merge, and kept out of the PR body so it never fights your team's PR template. One scoped change gets one plan file and one branch, never one plan for a whole initiative; a large ask was split into separate scoped changes upstream, each with its own plan.
...
The log is one file per decision, which keeps it rebase-friendly: two branches adding decisions never touch the same file. Read `adr_dir` from `.tests-as-specs/config.json` (default `.tests-as-specs/adrs`), then write each entry to `<adr_dir>/<date>-<slug>.md`, where `<date>` is today and `<slug>` is a short kebab-case name for the decision. Per file:
